...
public sealed class UpdateClientCompanyRequestValidator : AbstractValidator<UpdateClientCompanyRequest>
{
    ...
    {
        RuleFor(x => x.Code).NotEmpty().WithMessage("Le code société est obligatoire.").MaximumLength(20);
        RuleFor(x => x.RaisonSociale).NotEmpty().WithMessage("La raison sociale est obligatoire.").MaximumLength(200);
